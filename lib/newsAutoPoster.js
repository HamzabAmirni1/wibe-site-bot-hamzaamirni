...
async function postToFacebook(text, imageUrl = null) {
    ...
    try {
        let postId = null;
        let finalImg = imageUrl || DEFAULT_TECH_IMAGE;
        ...
        try {
            ...
            console.error(`[newsAutoPoster] Photo post failed, fallback to text:`, imgErr.response?.data?.error?.message || imgErr.message);
        ...
        if (postId) console.log(`[newsAutoPoster] FB Post successful: ${postId}`);
    } catch (e) {
        console.error(`[newsAutoPoster] FB Total Failure:`, e.response?.data?.error?.message || e.message);
    }
}
