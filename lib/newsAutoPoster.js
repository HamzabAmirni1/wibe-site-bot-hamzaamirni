...
async function scrapeContent(url) {
    try {
        ...
        // Try to get full text from common article bodies
        let fullText = "";
...
async function postComment(postId, pageToken, message) {
    try {
        ...
        console.error(`[newsAutoPoster] Comment failed:`, e.response?.data || e.message);
    }
}
...
async function postToFacebook(text, imageUrl = null) {
    ...
    for (const page of pages) {
        try {
            ...
            console.log(`[newsAutoPoster] Posted to FB Page: ${page.id} (ID: ${postId})`);
            
            // Add site link in first comment
            if (postId) {
                const commentMsg = `🔗 تصفح موقعي الرسمي لمزيد من المحتوى الحصري:\n${config.portfolio}`;
